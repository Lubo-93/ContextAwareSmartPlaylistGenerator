...
/**
 * Created by Lyubomir on 10/04/2015.
 *
...
public class GeofenceRemover implements
    ...
    private AppParams.REMOVE_TYPE mRequestType;
    ...
    public void removeGeofencesById(List<String> geofenceIds) throws IllegalArgumentException, UnsupportedOperationException {
        ...
        if ((null == geofenceIds) || (geofenceIds.size() == 0)) {
            ...
            if (!mInProgress) {
                mRequestType = AppParams.REMOVE_TYPE.LIST;
    ...
    public void removeGeofencesByIntent(PendingIntent requestIntent) {
        ...
        if (!mInProgress) {
            // Set the request type, store the List, and request a location client connection.
            mRequestType = AppParams.REMOVE_TYPE.INTENT;
    ...
    public void onRemoveGeofencesByPendingIntentResult(int statusCode, PendingIntent requestIntent) {
        ...
        if (statusCode == LocationStatusCodes.SUCCESS) {
            ...
            broadcastIntent.setAction(AppParams.ACTION_GEOFENCES_REMOVED);
            broadcastIntent.putExtra(AppParams.EXTRA_GEOFENCE_STATUS, mContext.getString(R.string.remove_geofences_intent_success));
            ...
            mLog.addMessage(status);
            // Set the action and add the result message
            broadcastIntent.setAction(AppParams.ACTION_GEOFENCE_ERROR);
            broadcastIntent.putExtra(AppParams.EXTRA_GEOFENCE_STATUS, mContext.getString(R.string.remove_geofences_intent_failure, statusCode));
        }
    ...
    public void onRemoveGeofencesByRequestIdsResult(int statusCode, String[] geofenceRequestIds) {
        ...
        if (LocationStatusCodes.SUCCESS == statusCode) {
            ...
            broadcastIntent.setAction(AppParams.ACTION_GEOFENCES_REMOVED).addCategory(AppParams.CATEGORY_CONTEXT_ANALYSIS)
                    .putExtra(AppParams.EXTRA_GEOFENCE_STATUS, msg);
            // If removing the geocodes failed
        } else {
            ...
            mLog.addMessage(status);
            // Create an Intent to broadcast to the app
            broadcastIntent.setAction(AppParams.ACTION_GEOFENCE_ERROR).addCategory(AppParams.CATEGORY_CONTEXT_ANALYSIS)
                    .putExtra(AppParams.EXTRA_GEOFENCE_STATUS, msg);
        }
    ...
    private void requestDisconnection() {
         ...
         */
        if (mRequestType == AppParams.REMOVE_TYPE.INTENT) {
    ...
    public void onConnectionFailed(ConnectionResult connectionResult) {
        ...
        if (connectionResult.hasResolution()) {
            ...
            try {
                ...
                connectionResult.startResolutionForResult((Activity) mContext, AppParams.CONNECTION_FAILURE_RESOLUTION_REQUEST);
         ...
         */
            ...
            Intent errorBroadcastIntent = new Intent(AppParams.ACTION_CONNECTION_ERROR);
            errorBroadcastIntent.addCategory(AppParams.CATEGORY_CONTEXT_ANALYSIS)
                    .putExtra(AppParams.EXTRA_CONNECTION_ERROR_CODE, connectionResult.getErrorCode());
